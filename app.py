...
PETTY_EXCLUDE_MONTHS = set()

# Authoritative sheet names in the Petty Cash Excel workbook (Sep 2025 – Aug 2026)
PETTY_CASH_SHEETS = [
...
GITHUB_EXCEL_URL = (
    ...
    "Zen_Estate_Combined_Expenses_Q1.xlsx"
)


def _fetch_main_excel_bytes():
    """Prefer local workbook beside app.py; fall back to GitHub."""
    from io import BytesIO
    from pathlib import Path

    import requests

    local_path = Path(__file__).resolve().parent / "Zen_Estate_Combined_Expenses_Q1.xlsx"
...
def load_excel_from_github():
    """Load Excel file from local copy or GitHub repository"""
    try:
        from io import BytesIO

        excel_file = BytesIO(_fetch_main_excel_bytes())
...
def load_leela_data():
    ...
    try:
        ...
        items = []
        seen = set()
        for row in range(df_sheet5.shape[0]):
            # Description in col C (index 2), Amount in col D (index 3)
            desc = df_sheet5.iloc[row, 2] if df_sheet5.shape[1] > 2 else None
            amt = df_sheet5.iloc[row, 3] if df_sheet5.shape[1] > 3 else None
            if pd.notna(desc) and isinstance(desc, str) and desc.strip():
...
def render_petty_cash(petty_data, fy_start):
    ...
    month_colors = {
        ...
        "Jun 2026": "#ffd6cc", "Jul 2026": "#ccf5ff", "Aug 2026": "#e9d5ff",
    }
